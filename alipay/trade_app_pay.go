...
type TradeAppPayRequest struct {
	...
	GoodsDetail []*GoodsDetailItem `json:"goods_detail,omitempty"` // 订单包含的商品列表信息，Json格式，详见商品明细说明
}

func (s *TradeAppPayRequest) APIName() string {
	return "alipay.trade.app.pay"
}

func (s *TradeAppPayRequest) Params() map[string]string {
	var m = make(map[string]string)
	if s.AppAuthToken != "" {
		m["app_auth_token"] = s.AppAuthToken
	}
	if s.NotifyURL != "" {
		m["notify_url"] = s.NotifyURL
	}
	if s.ReturnURL != "" {
		m["return_url"] = s.ReturnURL
	}
	return m
}
